...
TEST_KOLIBRI_SNAPSHOT_SRC := tests/test_kolibri_ai_snapshot.c src/kolibri_ai.c src/formula_runtime.c src/synthesis/search.c
...
.PHONY: test test-vm test-fkv test-config test-kolibri-ai test-kolibri-ai-snapshot test-http-routes bench clean run build



test: build test-vm test-fkv test-config test-kolibri-ai test-kolibri-ai-snapshot test-swarm-protocol
...
$(BUILD_DIR)/tests/test_kolibri_ai_snapshot: $(TEST_KOLIBRI_SNAPSHOT_SRC)
	@mkdir -p $(BUILD_DIR)/tests
	$(CC) $(CFLAGS) $(TEST_KOLIBRI_SNAPSHOT_SRC) -o $@ $(LDFLAGS)

test-kolibri-ai-snapshot: $(BUILD_DIR)/tests/test_kolibri_ai_snapshot
	$<
