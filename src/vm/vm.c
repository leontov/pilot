...
int vm_run(const prog_t *p, const vm_limits_t *lim, vm_trace_t *trace, vm_result_t *out) {
    ...
    while (ip < p->len) {
        ...
        case 0x0C: { // READ_FKV
            ...
            const size_t max_key_digits = 32;
            if (validate_decimal_operand(key_num, max_key_digits) != 0) {
                status = VM_ERR_INVALID_OPCODE;
                goto done;
            }
            uint8_t key_digits[32];
            size_t key_len = max_key_digits;
        ...
        case 0x0D: { // WRITE_FKV
            ...
            if (validate_decimal_operand(key_num, sizeof(key_digits)) != 0 ||
                validate_decimal_operand(value_num, sizeof(value_digits)) != 0 ||
                number_to_digits(key_num, key_digits, &key_len) != 0 ||
