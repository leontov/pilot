...
static int parse_program_array(const char *body, size_t body_len, struct json_object **out_root, struct json_object **out_program) {
    struct json_object *root = NULL;
    int rc = parse_json_object(body, body_len, &root);
    if (rc != PARSE_OK) {
        return rc;
    }
    struct json_object *program_obj = NULL;
    if (!json_object_object_get_ex(root, "program", &program_obj)) {
        json_object_put(root);
        return PARSE_ERR_MISSING_FIELD;
    }
